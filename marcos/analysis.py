...
def load_data(file, *args, **kwargs):
    ...
    if abf.channelCount == 1:
        ...
        if 'interval' in kwargs:
            ...
            del kwargs['interval']
        return load_double_channel(file, *args, **kwargs)
    else:
        raise NotImplementedError(f"Can't handle files with {abf.channelCount} channels")

def load_double_channel(file, gauss_filter=False, override_raw=False):
    """
...
def load_single_channel(file, interval=None, gauss_filter=False, override_raw=False):
    """
...
def add_run_info(data, file):
    ...
    pair_guide_file = file.parent / 'info.xlsx'
    pair_guide = pd.read_excel(pair_guide_file)
    pair_guide['rec'] = pair_guide.name.astype(str)
    # pair_guide = pair_guide.set_index('name')
    
    run_info = next(pair_guide[pair_guide.rec == file.stem].itertuples())
